...
const ChiTietDonKham = () => {
  ...
  const loadDonKham = async () => {
    try {
      setIsLoadDK(true);
      const res = await authApis().get(endpoints.donkhamuser + `/${id}`);
      if (res.status != 200)
        alert("load đơn khám " + id + " thất bại");
      else {
        setDonKham(res.data);
      }
    } catch (e) {
  ...
  const loadChiTiet = async ()=>{
     try {
      setIsLoadCT(true);
      const res = await authApis().get(endpoints.chitietdonkham+`?donKhamId=${id}`);
      if (res.status != 200)
  ...
  const loadXetNghiem = async ()=>{
     try {
      setIsLoadXN(true);
      const res = await authApis().get(endpoints.xetnghiem+`?donKhamId=${id}`);
      if (res.status != 200)
  ...
  return (
    ...
    <h3>Đơn khám :</h3>
      <ListGroup style={{ margin: "10px" }}>
        ...
        {!donKham.isPaid && <ListGroup.Item style={lGI1}>Chưa thanh toán </ListGroup.Item>}
        {donKham.isPaid && <ListGroup.Item style={lGI}>Đã thanh toán </ListGroup.Item>}
      </ListGroup>
      ...
      {!donKham.isPaid && <Button variant="warning" onClick={()=> navigate(`/thanhtoan/${donKham.id}`)} >Thanh toán</Button>}
    </>
  );
};
const lGI = {
